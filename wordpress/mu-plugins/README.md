...
| `sasanperfumes-cocart-coupons.php` | Adds the CoCart coupon endpoints the storefront calls. CoCart v2 ships none, so coupons used to go through the Store API, which opens its own cart: the code applied to an empty throwaway basket and the customer's cart never saw it. Also saves the session in the blog CoCart booted on — CoCart writes with the live `$wpdb->prefix` but reads a table name captured at load, so on `/qa`, `/om` and `/sa` the write went to a per-market table that does not exist, and every coupon was lost on the next read. |
